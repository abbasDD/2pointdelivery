...
<div class="card">
    ...
    <form action="{{ route('helper.update.address') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="card-body">
            <div class="row">
                {{-- Suite --}}
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="suite" name="suite"
                            value="{{ old('suite', $helperData['suite'] ?? '') }}" placeholder="Suite " required>
                        @error('suite')
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="street" name="street"
                            value="{{ old('street', $helperData['street'] ?? '') }}" placeholder="Street" required>
                        @error('street')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                        ...
                        <select class="form-control @error('country') is-invalid @enderror" id="country"
                            ...
                            @foreach ($addressData['countries'] as $country)
                                <option value="{{ $country->id }}"
                                    {{ old('country', $helperData['country'] ?? '') == $country->id ? 'selected' : '' }}>
                                    {{ $country->name }}</option>
                            @endforeach
                        </select>
                        @error('country')
                ...
                <div class="col-md-6">
                    <div class="form-group mb-3">
                        ...
                        <select class="form-control @error('state') is-invalid @enderror" id="state" name="state"
                            ...
                            @foreach ($addressData['helperStates'] as $state)
                                <option value="{{ $state->id }}"
                                    {{ old('state', $helperData['state'] ?? '') == $state->id ? 'selected' : '' }}>
                                    {{ $state->name }}</option>
                            @endforeach
                        </select>
                        @error('state')
                ...
                <div class="col-md-6">
                    <div class="form-group mb-3">
                        ...
                        <select class="form-control @error('city') is-invalid @enderror" id="city" name="city"
                            ...
                            @foreach ($addressData['helperCities'] as $city)
                                <option value="{{ $city->id }}"
                                    {{ old('city', $helperData['city'] ?? '') == $city->id ? 'selected' : '' }}>
                                    {{ $city->name }}</option>
                            @endforeach
                        </select>
                        @error('city')
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="zip_code" name="zip_code"
                            value="{{ old('zip_code', $helperData['zip_code'] ?? '') }}" placeholder="Zip Code "
                            required>
                        @error('zip_code')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
