    ...
    <tbody>
        @forelse ($helpQuestions as $helpQuestion)
            <tr>
                ...
                <td>{!! Str::limit($helpQuestion->answer ?? '-', 200) !!}</td>
                <td>
                    <button type="button" id="statusButton_{{ $helpQuestion->id }}"
                        class="btn  {{ $helpQuestion->is_active ? 'btn-primary' : 'btn-danger' }} btn-sm"
                        onclick="showStatusDialog({{ $helpQuestion->id }}, '{{ $helpQuestion->is_active }}')">
                        {{ $helpQuestion->is_active == 1 ? 'Active' : 'Inactive' }}
                    </button>
                </td>
                <td><a href="{{ route('admin.helpQuestion.edit', $helpQuestion->id) }}"><i class="fa fa-edit"></i></a>
                </td>
            </tr>
        @empty
            <tr>
...
<script>
    function showStatusDialog(id, status) {
        ...
        var baseUrl = "{{ url('admin/help-question/update-status') }}"; // Use `url()` to get the base part
    ...
    function updateStatus(id) {
        console.log(id);
        var baseUrl = "{{ url('admin/help-question/update-status') }}";
