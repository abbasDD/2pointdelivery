    ...
    <div class="col-md-6">
        <div class="form-group mb-3">
            ...
            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name"
                value="{{ old('name', $floorAssess['name'] ?? '') }}" placeholder="Enter Name" required>
            @error('name')
        ...
        <div class="form-group mb-3">
            ...
            <div class="input-group">
                <input type="text" id="price" class="form-control" placeholder="Price" name="price"
                    value="{{ old('price', $floorAssess['price'] ?? '') }}" aria-describedby="price"
    ...
    <div class="col-md-12">
        <div class="form-group mb-3">
            ...
            <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
                rows="3" placeholder="Enter Description" required>{{ old('description', $floorAssess['description'] ?? '') }}</textarea>
            @error('description')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
        ...
        <div class="form-group mb-3">
            ...
            <div class="input-group">
                <input type="text" id="helper_fee" class="form-control" placeholder="Helper Fee" name="helper_fee"
                    value="{{ old('helper_fee', $floorAssess['helper_fee'] ?? '') }}" aria-describedby="helper_fee"
    ...
    <div class="col-md-6">
        <div class="form-group mb-3">
            ...
            <select class="form-control @error('is_active') is-invalid @enderror" id="is_active" name="is_active"
                ...
                <option value="1" {{ old('is_active', $floorAssess['is_active'] ?? '') == 1 ? 'selected' : '' }}>
                    Active
                </option>
                <option value="0" {{ old('is_active', $floorAssess['is_active'] ?? '') == 0 ? 'selected' : '' }}>
                    Inactive
                </option>
            </select>
            @error('is_active')
